...
def generate_testimonials(test_name, city_name):
...
def dummy(test_name, city_name):
    st.write(f"""
             Test name: {test_name}
             City name: {city_name}
             Testimonial Creation is in Progress""")
    result = generate_testimonials(test_name, city_name)
    st.write(result)
...
def main():
    ...
    if st.button("Create Testimonials"):
        if test_name and city_name and openai_key:
            openai.api_key = openai_key
            with st.spinner("Creating Testimonials ...."):
                dummy(test_name, city_name)
        else:
